...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        /*ordenação mergesort sort - START*/
        Merge mergesortObject = new Merge();

        Series[] sortMethodArray = mergesortObject.mergesort(sortSeriesArray, 0, inputCount); //recebe o vetor ordenado

        for (int i = 0; i < seriesQtd; i++) {
            sortMethodArray[i].printer(sortMethodArray[i]); //leitura do vetor ordenado
        }
        /*ordenação mergesort sort - END*/

        /*Arquivo log de armazenamento - START*/
        long executionTime = System.currentTimeMillis();

        TextFileWrite writeFile = new TextFileWrite("752652_mergesort.txt"); //cria arquivo log

        writeFile.write("Numero de trocas entre elementos: " + mergesortObject.countSwitch + " Numero de movimentacoes no vetor " + mergesortObject.countCompare + "Matricula: xxxxx Tempo de execução em milisegundos: " + executionTime);
