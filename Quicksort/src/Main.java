...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        /*ordenação quicksort - START*/
        Quick quickSortObject = new Quick();

        Series[] sortMethodArray = quickSortObject.quicksort(sortSeriesArray, 0, inputCount); //recebe o vetor ordenado

        for (int i = 0; i < seriesQtd; i++) {
            sortMethodArray[i].printer(sortMethodArray[i]); //leitura do vetor ordenado
        }
        /*ordenação quicksort - END*/

        /*Arquivo log de armazenamento - START*/
        long executionTime = System.currentTimeMillis();

        TextFileWrite writeFile = new TextFileWrite("752652_quick.txt"); //cria arquivo log

        writeFile.write("Numero de trocas entre elementos: " + quickSortObject.countSwitch + " Numero de movimentacoes no vetor " + quickSortObject.countCompare + "Matricula: xxxxx Tempo de execução em milisegundos: " + executionTime);
